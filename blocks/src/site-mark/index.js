...
registerBlockType( metadata.name, {
	edit( { attributes, setAttributes } ) {
		const { href, label, size = 14 } = attributes;
		const blockProps = useBlockProps( {
			className: 'mh-site-mark',
			style: {
				display: 'inline-flex',
				width: size,
				height: size,
		...
		return (
			<>
				<InspectorControls>
					<PanelBody
						title={ __( 'Site Mark', 'mercantile-hook-loop' ) }
					>
						<TextControl
							label={ __(
								'Link URL (defaults to home)',
