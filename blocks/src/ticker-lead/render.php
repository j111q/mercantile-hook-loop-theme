 ...
 * pseudo on .mh-ticker__live (see style.css), so the rendered DOM
 * stays minimal and there is no empty `<span>` shell.
 *
...
$label     = isset( $attributes['ariaLabel'] ) ? (string) $attributes['ariaLabel'] : 'Mercantile home';
$live_text = isset( $attributes['liveText'] ) && '' !== $attributes['liveText'] ? (string) $attributes['liveText'] : 'LIVE';
$size      = isset( $attributes['size'] ) ? (int) $attributes['size'] : 14;

$wrapper_attrs = get_block_wrapper_attributes( array( 'class' => 'mh-ticker__lead' ) );
...
printf(
	'<div %1$s><a class="mh-site-mark" href="%2$s" aria-label="%3$s" title="%3$s">%4$s</a><span class="mh-ticker__live">%5$s</span></div>',
	$wrapper_attrs,
	$href,
	esc_attr( $label ),
	$svg,
